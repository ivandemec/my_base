...
def extract_tags(content):
    ...
    # Inline #tags in the body (ignore code spans and markdown headings).
        ...
        for file in files:
            if file.endswith('.md'):
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    note_tags[file.lower()] = extract_tags(content)
...
def create_html():
    ...
    if not vault_dir:
        messagebox.showerror(
            "Error", "Please select the vault directory first.")
        return
    if not output_dir:
        messagebox.showerror(
            "Error", "Please select the output directory first.")
        return
...
def select_vault_directory():
    ...
    if vault_dir:
        messagebox.showinfo("Vault Directory",
                            f"Vault directory selected: {vault_dir}")
...
def select_output_directory():
    ...
    if output_dir:
        messagebox.showinfo("Output Directory",
                            f"Output directory selected: {output_dir}")
...
btn_select_vault = tk.Button(
    frame, text="Select Vault Directory", command=select_vault_directory)
btn_select_vault.pack(pady=10)

btn_select_output = tk.Button(
    frame, text="Select Output Directory", command=select_output_directory)
...
btn_support = tk.Button(
    frame, text="Consider supporting me", command=open_support_link)
